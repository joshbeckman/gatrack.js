    ...
    var initProfile,
        ...
        tempTrack = {
            ...
            action: function(elem, category, action, label, value, callback) {
                ...
                // Rudimentary support for multiple profiles
                try {
             ...
             *
             * @param {DOM Element} elem
             * @param {String} scrollPoint Percentage or pixels scrolled at which to trigger
            ...
            scrollAt: function(elem, scrollPoint, direction, category, action, label, value) {
                var px,
                    ...
                    act = action || elem[ds][gaTA] || elem.title ||
                        elem.id || doc[gEBTN]('title')[0].textContent || point,
                    lab = label || elem[ds][gaTL] || direct,
                    val = value || elem[ds][gaTV];

                if (point.slice(-1) == '%') {
                    px = val = (direct == 'y' ? elem.clientHeight : elem.clientWidth) *
                        parseInt(point.slice(0, (point.length - 2))) / 100;
            ...
            /**
             * onerror listens for the native onerror
             *
             */
            onerror: {
                push: function(errArray) {
                    var cat = 'Recorded Error',
                        lab = errArray[0],
                        err = errArray[4],
                        act = ('Error line:col(url) ' +
                            (errArray[2] ? errArray[2].toString() : '') +
                            ':' + (errArray[3] ? errArray[3].toString() : '') +
                            '(' + errArray[1] + ')'),
                        val = Math.round(((new Date()).getTime() - tempTrack.timer) / 100) / 10;

                    // Record value as time (in seconds, ronded to nearest hundreth)
